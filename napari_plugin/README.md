...
Status: phase 3 of [PLAN.md](PLAN.md). The package installs, registers a napari reader for `.zarr` paths and URLs, opens any node of an sp-ops store from a single channel image up to the screen root, and adds a navigator dock widget for picking wells and tiles. Layers carry channel names, colormaps and contrast limits from `omero` or the role palette, additive blending, a `round` slider, tile placement from the `layout` polygons, and cell-table columns as label features. Groups outside an sp-ops store go to napari-ome-zarr unchanged.
...
Image layers blend additively, so the channels of a tile compose into one picture. Colormaps and contrast limits come first from the node's OME `omero` block, channel by channel: `channels[i].color` becomes the colormap and `channels[i].window.start` and `end` the contrast limits. A channel without an `omero` entry takes the role palette, and its contrast limits are estimated from the lowest pyramid level when that level has at most about a million elements. Larger images with no window leave contrast to napari.
